classdef Hex < handle
    ...
    methods
        ...
        % board:       11x11 matrix with 0=empty, 1=red, 2=blue
        ...
        % =================================================================
        function g = Hex(initialboard)
            % Constructor
            nCells = sqrt(size(g.adj,1) - 4);
            
            if nargin < 1
                initialboard = zeros(nCells);
            end
        ...
        function side = whoseMove(game)
            ...
            % next.
            if sum(game.board(:)==1) == sum(game.board(:)==2)
                side = 1;
            else
                side = 2;
            end
        ...
        function r = isGameOver(game)
            ...
            a = game.adj;
            side1 = [find(game.board==1); nCellsSq+1; nCellsSq+2];
            side = side1;
            len = length(side);
            a = a(side,side);
            ...
            % Test for Side 1 victory
            a = game.adj;
            side2 = [find(game.board==2); nCellsSq+3; nCellsSq+4];
            side = side2;
            len = length(side);
            a = a(side,side);
            dist = graphshortestpath(a,len-1,len);
